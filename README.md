...
**建置狀態**：✅ 買家前台 14 階段完成 + 賣家後台 9 模組完成（2026/06/14，含預約管理與權限管理）+ 預約子系統 6 頁面完成（2026/06/12）
...
│   │   ├── movement.js           # 庫存異動紀錄：配送店 / 接收店異動表格
...
│       ├── movement.json         # 20 筆庫存異動主檔（含異動明細清單）
...
- `v1.2.5`（2026/06/13）：庫存異動紀錄改為明細清單；商品頁「生產異動紀錄」整合。
