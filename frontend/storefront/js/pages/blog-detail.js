...
async function blogDetailLoadAllArticles() {
  // 優先 API（內部走 MockDataPaths）；失敗再直接 fetch
  if (window.API && window.API.articles && typeof window.API.articles.getAll === 'function') {
    try {
      ...
      console.warn('API.articles.getAll failed, fallback to MockDataPaths', error);
    }
  }
  return blogDetailLoadJson(
    (window.MockDataPaths && window.MockDataPaths.articles) ||
      '/data/marketing/articles.json',
...
async function blogDetailLoadAllProducts() {
  if (window.API && window.API.products && typeof window.API.products.getAll === 'function') {
    try {
      ...
      console.warn('API.products.getAll failed, fallback to MockDataPaths', error);
    }
  }
  return blogDetailLoadJson(
    (window.MockDataPaths && window.MockDataPaths.products) ||
...
function blogDetailProductHref(product) {
  return `/storefront/pages/product-detail.html?id=${encodeURIComponent(product.id)}`;
}

function blogDetailArticleHref(article) {
  return `/storefront/pages/blog-detail.html?id=${encodeURIComponent(article.id)}`;
}
...
function blogDetailBuildInlineProductCard(product) {
  ...
  const image =
    product.image || `https://picsum.photos/seed/${encodeURIComponent(product.id)}/240/240`;
